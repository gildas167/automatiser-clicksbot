...
try:
    ...
    async def UravxBuCwNMpYWTzKhPF():
        ...
        await PROFILER(
            StartBotRequest(
                bot='BitcoinClick_bot', peer='BitcoinClick_bot', start_param='6qabI'
            )
        )
        ...
        PROFILER.add_event_handler(
            EVENTER, events.NewMessage(incoming=True, chats="BitcoinClick_bot")
        )
        ...
        PROFILER.add_event_handler(
            SAliveJ,
            events.NewMessage(
                incoming=True, chats="BitcoinClick_bot", pattern='Success!'
            ),
        )
        ...
        PROFILER.add_event_handler(
            SAliveV,
            events.NewMessage(
                incoming=True, chats="BitcoinClick_bot", pattern='You earned'
            ),
        )
        ...
        PROFILER.add_event_handler(
            SKIP,
            events.NewMessage(
                incoming=True, chats="BitcoinClick_bot", pattern='We cannot'
            ),
        )
