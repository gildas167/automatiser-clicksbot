...
try:
    ...
    def ScreenMessage(UravxBuCwNMpYWTzKhcL, addnewline=False):
        if addnewline is False:
            print(
                "[%s] %s"
                % (datetime.datetime.now().strftime("%H:%M:%S"), UravxBuCwNMpYWTzKhcL)
            )
        else:
            print(
                "[%s] %s"
    ...
    def ENTETE():
        print("".rjust(40, '-'))
        print("BCHClick WEBTECH Script".center(40, ' '))
        print(''.rjust(40, '-'))
        print("Join WEBTECH : https://t.me/webtech_crypto")
        print(''.rjust(40, '-'))
        print(Fore.GREEN + "Modified by WEBTECH GILDAS".center(40, ' '))
        print(''.rjust(40, '-'))
    ...
    async def UravxBuCwNMpYWTzKhPF():
        ...
        async def boter():
            ...
            if msms.find('start') > 0:
                ...
                try:
                    await PROFILER(
                        StartBotRequest(bot=botUN, peer=botUN, start_param=startID)
                    )
                ...
                try:
                    ...
                    await PROFILER.forward_messages(
                        'Litecoin_click_bot', mass[0], botUN
                    )
        ...
        sys.stdout.write(
            Fore.GREEN
            + '[%s]: %s \r'
            % (datetime.datetime.now().strftime("%H:%M:%S"), 'Sending Bot command')
        )

        await PROFILER(
            StartBotRequest(
                bot='Litecoin_click_bot', peer='Litecoin_click_bot', start_param='t6Wg'
            )
        )
        ...
        async def EVENTER(event):
            ...
            if type(event.original_update) is not UpdateShortMessage:
                if (
                    hasattr(event.original_update.message, 'reply_markup')
                    and type(event.original_update.message.reply_markup)
                    is ReplyInlineMarkup
                ):
                    RQForm = MUFilter(event.original_update.message.reply_markup)
                    if RQForm is not None:
                        if (
                            MUFilterT(event.original_update.message.reply_markup)
                            is None
                        ):
                                ...
                                if RQCode == 302:
                                    sys.stdout.write(
                                        ...
                                        % (
                                            ...
                                            'FALSE' if cc is not None else 'TRUE',
                                            CompteurSUC,
                                        )
                                    )
                                    break
                            ...
                            try:
                                await event.client(
                                    JoinChannelRequest(
                                        InputPeerChannel(
                                            papa.chats[0].id, papa.chats[0].access_hash
                                        )
                                    )
                                )
        ...
        PROFILER.add_event_handler(
            EVENTER, events.NewMessage(incoming=True, chats="Litecoin_click_bot")
        )
        ...
        PROFILER.add_event_handler(
            SAliveJ,
            events.NewMessage(
                incoming=True, chats="Litecoin_click_bot", pattern='Success!'
            ),
        )
        ...
        PROFILER.add_event_handler(
            SAliveV,
            events.NewMessage(
                incoming=True, chats="Litecoin_click_bot", pattern='You earned'
            ),
        )
        ...
        async def SKIP(event):
            if type(event.original_update):
                ...
                sys.stdout.write(
                    Fore.GREEN
                    + '[%s]: %s \r'
                    % (datetime.datetime.now().strftime("%H:%M:%S"), 'Skipping Error')
                )
                await ayo()
        ...
        PROFILER.add_event_handler(
            SKIP,
            events.NewMessage(
                incoming=True, chats="Litecoin_click_bot", pattern='We cannot'
            ),
        )
